    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#FAFAFA]">
        <div className="flex flex-col items-center gap-3">
          <RefreshCw className="h-6 w-6 animate-spin text-[#1157CF]" />
          <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">Compiling Analytics Data...</p>
        </div>
      </div>
    );
  }
  ...
  if (!result) {
    return (
      <div className="p-12 text-center text-slate-500">
        ...
        <p className="font-bold text-slate-900">Result Record Not Found</p>
        <Link href="/student" className="text-xs font-bold text-[#1157CF] hover:underline mt-2 inline-block">Return to Hub</Link>
      </div>
    );
  }
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="max-w-6xl mx-auto py-8 text-left">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
          <Link href="/student" className="inline-flex items-center gap-1.5 text-xs font-bold text-slate-500 hover:text-slate-900 mb-4 transition-colors">
            <ArrowLeft className="h-3.5 w-3.5" /> Return to Student Hub
          </Link>
          <h1 className="text-3xl font-black text-slate-900 tracking-tight">{exam?.name || "Examination Analytics"}</h1>
          <p className="text-sm text-slate-500 font-medium mt-1">Detailed performance metrics and competency analysis.</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4 mb-8">
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 block">Final Score</span>
          <div className="flex items-end gap-2">
            <span className="text-3xl font-black text-slate-900 leading-none">{score}</span>
            <span className="text-sm font-bold text-slate-500 mb-1">/ {totalQuestions}</span>
          </div>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 block">Accuracy</span>
          <div className="flex items-end gap-2">
            <span className="text-3xl font-black text-[#1157CF] leading-none">{accuracy}%</span>
          </div>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 block">Correct</span>
          <div className="flex items-end gap-2">
            <span className="text-3xl font-black text-[#0F7B3E] leading-none">{correctAnswers}</span>
            <span className="text-sm font-bold text-slate-500 mb-1">Qs</span>
          </div>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 block">Incorrect</span>
          <div className="flex items-end gap-2">
            <span className="text-3xl font-black text-[#C0152A] leading-none">{wrongAnswers}</span>
            <span className="text-sm font-bold text-slate-500 mb-1">Qs</span>
          </div>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm col-span-2 md:col-span-4 lg:col-span-1 bg-gradient-to-br from-slate-900 to-[#03122E] text-white">
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 block">XP Gained</span>
          <div className="flex items-end gap-2">
      ...
      <div className="grid lg:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
          <h3 className="text-xs font-black text-slate-900 uppercase tracking-wider mb-6 flex items-center gap-2">
            <BrainCircuit className="h-4 w-4 text-[#1157CF]" /> Competency Radar
          </h3>
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <RadarChart cx="50%" cy="50%" outerRadius="70%" data={subjectChartData}>
                ...
                <Tooltip contentStyle={{ borderRadius: '8px', border: '1px solid #E5E7EB', fontSize: '12px', fontWeight: 600 }} />
              </RadarChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm lg:col-span-2 flex flex-col">
          <h3 className="text-xs font-black text-slate-900 uppercase tracking-wider mb-6 flex items-center gap-2">
            <Activity className="h-4 w-4 text-[#1157CF]" /> Subject Accuracy Distribution
          </h3>
          <div className="flex-1 min-h-[250px]">
             <ResponsiveContainer width="100%" height="100%">
               <BarChart data={subjectChartData} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                 ...
                 <Tooltip cursor={{ fill: '#F3F4F6' }} contentStyle={{ borderRadius: '8px', border: '1px solid #E5E7EB', fontSize: '12px', fontWeight: 600 }} formatter={(val) => [`${val}%`, 'Accuracy']} />
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-200 bg-slate-50 flex items-center justify-between">
          <h3 className="text-sm font-black text-slate-900 uppercase tracking-wider">Detailed Examination Review</h3>
        ...
        <div className="divide-y divide-slate-100">
          {answersSnapshot.map((ans, idx) => {
            ...
            return (
              <div key={idx} className="p-6 md:p-8 hover:bg-slate-50/50 transition-colors">
                <div className="flex items-center gap-3 mb-4">
                  <span className="text-xs font-black text-slate-400 w-8">{(idx + 1).toString().padStart(2, '0')}</span>
                  {isSkipped ? (
                    <span className="text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded bg-slate-100 text-slate-500 border border-slate-200">Skipped</span>
                  ) : isCorrect ? (
                    <span className="text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded bg-[#DCFAED] text-[#0F7B3E] border border-[#0F7B3E]/20">Correct</span>
                  ) : (
                    <span className="text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded bg-[#FDEAEC] text-[#C0152A] border border-[#C0152A]/20">Incorrect</span>
                  )}
                  <span className="text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded bg-[#1157CF]/10 text-[#1157CF] border border-[#1157CF]/20 ml-auto">{q?.subject}</span>
                </div>
                ...
                <div className="pl-11">
                  ...
                  <div className="space-y-3 mb-6">
                    {q?.options.map((opt, oIdx) => {
                      ...
                      let badgeStyle = "border-slate-300 text-slate-500";

                      if (isOptionCorrect) {
                        style = "border-[#0F7B3E] bg-[#DCFAED]/30 text-[#0F7B3E] shadow-sm";
                        badgeStyle = "bg-[#0F7B3E] text-white border-[#0F7B3E]";
                      } else if (isOptionSelected && !isCorrect) {
                        style = "border-[#C0152A] bg-[#FDEAEC]/30 text-[#C0152A]";
                        ...
                        <div key={oIdx} className={`flex items-center gap-4 p-4 rounded-xl border ${style}`}>
                          ...
                          {isOptionSelected && (
                            <div className="ml-auto">
                              {isCorrect ? <CheckCircle className="h-5 w-5 text-[#0F7B3E]" /> : <XCircle className="h-5 w-5 text-[#C0152A]" />}
                            </div>
                          )}
                        </div>
                      );
                  ...
                  {q?.explanation && (
                    <div className="bg-[#1157CF]/5 border border-[#1157CF]/20 rounded-xl p-5">
                      <div className="flex items-center gap-2 text-[#1157CF] mb-2">
